...
Let $c \notin \mathbb {M}$ and let $z_{c,k} > 2$ with $k \in \mathbb {N}$ be first element in the sequence that is greater than 2. Then, according to the algorithm described above, $z_{c,k} > 2$ is the last number that got calculated while iterating $\(z_{c,n}\)$ from $z=0$. 
